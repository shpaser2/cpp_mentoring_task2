...
//TODO add test with big file (40k+ chars)
        //R & W notificate each other which buffer to use.
        //buffer states: empty (busy or really empty and free)
                       //full (ready to write to file)
...
bool readyToStopWriting(Buffer& buffer1, Buffer& buffer2) {
    ...
    {
        std::unique_lock<decltype(buffer1.mtx)> lock(buffer1.mtx);
    ...
    {
        std::unique_lock<decltype(buffer2.mtx)> lock(buffer2.mtx);
